        ...
        didSet {
            if (shadow) {
                ...
                if orientationToAngle {
                    ...
                    OMLog.printd("\(layer.name ?? "")):shadowOffset: \(layer.shadowOffset) angle:\(CPCAngle.format(angle)))")
                }
            } else {
...
open class CPStepData : CustomDebugStringConvertible {
    ...
    class func setUpStepLayerGeometry(element:CPCElement<CALayer>,
        ...
        OMLog.printd("\(element.layer.name ?? "") : Angle \(CPCAngle.format(angle))) position in angle :\(element.anglePosition)")
        ...
        OMLog.printd("\(element.layer.name ?? "") : Frame \(positionInAngle.integral) from the aligned step angle \(CPCAngle.format(angle)) and the text size \(sizeOf.integral()))")
        element.layer.frame = positionInAngle
        if element.orientationToAngle {
            let rotationZ = (angle - startAngle)
            OMLog.printd("\(element.layer.name ?? "") : Image will be oriented to angle: \(CPCAngle.format(rotationZ))")
